...
class CentralStore:
    ...
    def search_knowledge(self, term: str, limit: int = 50, project: str | None = None,
        ...
        """Full-text-ish lookup. `project` scopes to that project + global entries;
        omit it to search the whole fleet (cross-project learnings are often the point)."""
        like = f"%{term}%"
    ...
    def count_knowledge(self, project: str) -> int:
        row = self.conn.execute(
            "SELECT COUNT(*) AS n FROM knowledge WHERE project=? OR project=''", (project,)
        ).fetchone()
        return int(row["n"])

    def knowledge_topics(self, project: str | None = None) -> list[tuple[str, int]]:
    ...
    def knowledge_brief(self, project: str, pinned_limit: int = 8,
        ...
        rows = db.rows_to_dicts(self.conn.execute(
            "SELECT * FROM knowledge WHERE project=? OR project='' ORDER BY ts DESC",
            (project,),
        ).fetchall())
