...
def _gates_section(project: ProjectSpec) -> str:
    ...
    lines += [
        ...
        "```",
        "",
        "Once you have been BLOCKED, address the request by its number instead — the block",
        "prints it, the OS already has the command string, and re-typing that string into an",
        "argument is how a worker in a git worktree finds its own way out refused by the",
        "isolation guard:",
        "",
        "```bash",
        ...
        'jarvis gate contest "$JARVIS_WO_ID" "<the exact command>" \\',
