...
def check_work_lands(store: ProjectStore) -> Iterator[Violation]:
    ...
    **IT TRUSTS `pr_url`, AND INV-PR-RECORDED IS WHY IT MAY.** The two are a chain: that
    check holds `pr_url` populated for every settled order that wrote code, so an order
    silent here for want of a pull request has either produced nothing or been reported
    there. Read either one alone and the scope looks like a hole; they cover it together.
...
def check_pull_request_recorded(store: ProjectStore) -> Iterator[Violation]:
    ...
    **THE STALE `pr_url` IS OUT OF SCOPE**, and deliberately: `wo-cd73c537` records #81,
    ...
    whether it is CURRENT is a `gh` round trip per settled order and belongs to
    `Daemon.discover_pull_requests`, not here. Filed as `bl-2aabaee8`.
