...
- `worker.autocompact_window` — how large a worker's conversation may grow before
  Claude Code compacts it, passed straight to `claude --autocompact`. **`400000` by
  ...
  into the bill on every call, and cache reads are 56% of what the fleet spends. Lower
  it for a project whose orders are short and repetitive, raise it for one whose work
  genuinely needs a longer memory (the CLI accepts `100000`–`1000000`), or set it to
  `null` to opt out and take the model's own window.
